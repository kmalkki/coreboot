...
static void i915_setup_egress_port(void)
{
	u32 reg32;
	u32 timeout;
...
static void i915_setup_dmi_rcrb(void)
{
	u32 reg32;
	u32 timeout;
	...
	if (i915_silicon_revision() == 1 && ((MCHBAR8(0xe08) & (1 << 5)) == 1)) {
...
static void i915_setup_pci_express_x16(void)
{
	u32 timeout;
	u32 reg32;
	u16 reg16;
	...
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x208);
	reg32 &= ~(1 << 8);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x208, reg32);

	/* We have no success with querying the usual PCIe registers
	 * for link setup success on the i915. Hence we assign a temporary
	...
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x224);
	reg32 &= ~(1 << 8);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x224, reg32);

	MCHBAR16(UPMC1) &= ~( (1 << 5) | (1 << 0) );

	/* Initialze PEG_CAP */
	reg16 = pci_mmio_read_config16(PCI_DEV(0, 0x01, 0), 0xa2);
	reg16 |= (1 << 8);
	pci_mmio_write_config16(PCI_DEV(0, 0x01, 0), 0xa2, reg16);
	...
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0xb4);
	reg32 &= 0x0007ffff;

	reg32 &= 0xfffe007f;

	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0xb4, reg32);

	/* Wait for training to succeed */
	printk(BIOS_DEBUG, "PCIe link training ...");
	timeout = 0x7ffff;
	while ((((pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x214) >> 16) & 4) != 3)  && --timeout) ;
	...
	if (reg32 != 0x00000000 && reg32 != 0xffffffff) {
		...
		reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x214);
		reg32 &= ~(0xf << 1);
		reg32 |=1;
		pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x214, reg32);

		reg16 = pci_mmio_read_config16(PCI_DEV(0, 0x01, 0), 0x3e);

		reg16 |= (1 << 6);
		pci_mmio_write_config16(PCI_DEV(0, 0x01, 0), 0x3e, reg16);
		reg16 &= ~(1 << 6);
		pci_mmio_write_config16(PCI_DEV(0, 0x01, 0), 0x3e, reg16);
	...
	reg16 = pci_mmio_read_config16(PCI_DEV(0, 0x01, 0), 0xb2);
	reg16 >>= 4;
	reg16 &= 0x3f;
	/* reg16 == 1 -> x1; reg16 == 16 -> x16 */
	printk(BIOS_DEBUG, "PCIe x%d link training succeeded.\n", reg16);

	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x204);
	reg32 &= 0xfffffc00; /* clear [9:0] */
	if (reg16 == 1) {
		reg32 |= 0x32b;
		// TODO
		/* pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x204, reg32); */
	} else if (reg16 == 16) {
		reg32 |= 0x0f4;
		// TODO
		/* pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x204, reg32); */
	...
	/* Enable GPEs */
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0xec);
	reg32 |= (1 << 2) | (1 << 1) | (1 << 0); /* PMEGPE, HPGPE, GENGPE */
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x114, reg32);

	/* Virtual Channel Configuration: Only VC0 on PCIe x16 */
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x114);
	reg32 &= 0xffffff01;
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x114, reg32);

	/* Extended VC count */
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x104);
	reg32 &= ~(7 << 0);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x104, reg32);
	...
	/* Clear error bits */
	pci_mmio_write_config16(PCI_DEV(0, 0x01, 0), 0x06, 0xffff);
	pci_mmio_write_config16(PCI_DEV(0, 0x01, 0), 0x1e, 0xffff);
	pci_mmio_write_config16(PCI_DEV(0, 0x01, 0), 0xaa, 0xffff);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x1c4, 0xffffffff);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x1d0, 0xffffffff);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x1f0, 0xffffffff);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x228, 0xffffffff);

	/* Program R/WO registers */
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x308);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x308, reg32);

	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x314);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x314, reg32);

	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x324);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x324, reg32);

	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x328);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x328, reg32);

	reg8 = pci_mmio_read_config8(PCI_DEV(0, 0x01, 0), 0xb4);
	pci_mmio_write_config8(PCI_DEV(0, 0x01, 0), 0xb4, reg8);

	/* Additional PCIe graphics setup */
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0xf0);
	reg32 |= (3 << 26);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0xf0, reg32);
	...
	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x200);
	reg32 &= ~(3 << 26);
	reg32 |= (2 << 26);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x200, reg32);

	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0xe80);
	if (i915_silicon_revision() >= 2) {
		reg32 |= (1 << 12);
	} else {
		reg32 &= ~(1 << 12);
	}
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0xe80, reg32);

	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0xeb4);
	reg32 &= ~(1 << 31);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0xeb4, reg32);

	reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0xfc);
	reg32 |= (1 << 31);
	pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0xfc, reg32);
	...
	if (i915_silicon_revision() >= 3) {
		...
		for (i=0; i<ARRAY_SIZE(reglist); i++) {
			reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), reglist[i]);
			reg32 &= 0x0fffffff;
			reg32 |= (2 << 28);
			pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), reglist[i], reg32);
		}
	}
...
disable_pciexpress_x16_link:
	...
	reg16 = pci_mmio_read_config16(PCI_DEV(0, 0x01, 0), BCTRL1);
	reg16 |= (1 << 6);
	pci_mmio_write_config16(PCI_DEV(0, 0x01, 0), BCTRL1, reg16);
	...
	timeout = 0x7fffff;
	for (reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x214);
...
static void i915_setup_root_complex_topology(void)
{
	u32 reg32;
	...
	if (pci_read_config8(PCI_DEV(0, 0x00, 0), DEVEN) & DEVEN_D1F0) {
		pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x158, DEFAULT_EPBAR);
		reg32 = pci_mmio_read_config32(PCI_DEV(0, 0x01, 0), 0x150);
		reg32 |= (1 << 0);
		pci_mmio_write_config32(PCI_DEV(0, 0x01, 0), 0x150, reg32);
	}
}
...
void i915_early_initialization(void)
{
...
void i915_late_initialization(void)
{
	i915_setup_egress_port();
	...
	i915_setup_pci_express_x16();

	i915_setup_root_complex_topology();
}
