...
typedef struct southbridge_intel_i82801fx_config config_t;

#if 0
static void i82801fx_enable_apic(struct device *dev)
{
	int i;
	u32 reg32;
...
static void i82801fx_enable_serial_irqs(struct device *dev)
{
...
static void i82801fx_pirq_init(device_t dev)
{
	device_t irq_dev;
...
static void i82801fx_gpi_routing(device_t dev)
{
	/* Get the chip configuration */
	config_t *config = dev->chip_info;
	u32 reg32 = 0;
...
static void i82801fx_power_options(device_t dev)
{
	u8 reg8;
	u16 reg16, pmbase;
	u32 reg32;
	...
	/* Set DEBUG_PERIODIC_SMIS in i82801fx.h to debug using
	 * periodic SMIs.
	 */
...
static void i82801fx_configure_cstates(device_t dev)
{
	u8 reg8;
...
static void i82801fx_rtc_init(struct device *dev)
{
	u8 reg8;
	int rtc_failed;
...
static void i82801fx_lock_smm(struct device *dev)
{
...
static void i82801fx_fixups(struct device *dev)
{
...
{
	printk(BIOS_DEBUG, "i82801fx: lpc_init\n");

	/* Set the value for PCI command register. */
	pci_write_config16(dev, PCI_COMMAND, 0x000f);

	/* IO APIC initialization. */
	i82801fx_enable_apic(dev);

	i82801fx_enable_serial_irqs(dev);

	/* Setup the PIRQ. */
	i82801fx_pirq_init(dev);

	/* Setup power options. */
	i82801fx_power_options(dev);

	/* Configure Cx state registers */
	i82801fx_configure_cstates(dev);

	/* Set the state of the GPIO lines. */
	//gpio_init(dev);

	/* Initialize the real time clock. */
	i82801fx_rtc_init(dev);
...
#if CONFIG_HAVE_SMI_HANDLER
	i82801fx_lock_smm(dev);
#endif

	i82801fx_fixups(dev);
}

static void i82801fx_lpc_read_resources(device_t dev)
{
	struct resource *res;
...
static struct device_operations device_ops = {
	.read_resources		= i82801fx_lpc_read_resources,
	.set_resources		= pci_dev_set_resources,
	.enable_resources	= pci_dev_enable_resources,
//	.init			= lpc_init,
//	.scan_bus		= scan_static_bus,
//	.enable			= i82801fx_enable,
	.ops_pci		= &pci_ops,
};
...
static const struct pci_driver ich7_ich7r_lpc __pci_driver = {
	...
	.device	= PCI_DEVICE_ID_INTEL_82801FB_LPC,
};
