...
void i82801fx_enable(device_t dev)
{
	device_t lpc_dev;
	unsigned index = 0;
	...
	if ((lpc_dev->vendor != PCI_VENDOR_ID_INTEL) ||
		(lpc_dev->device != PCI_DEVICE_ID_INTEL_82801FB_LPC)) {
		uint32_t id;
		id = pci_read_config32(lpc_dev, PCI_VENDOR_ID);
		if (id != (PCI_VENDOR_ID_INTEL |
				(PCI_DEVICE_ID_INTEL_82801FB_LPC << 16))) {
			return;
		}
	}
...
struct chip_operations southbridge_intel_i82801fx_ops = {
	CHIP_NAME("Intel ICH5 (82801Ex) Series Southbridge")
	.enable_dev = i82801fx_enable,
};
